-- Solo turns publish immediately after one provider call, but first store a
-- durable recovery task so an API interruption cannot lose generated prose.
-- Keep the database vocabulary in sync with the already deployed Solo task
-- producers and worker outcomes.
...
CHECK ("taskType" IN (
    ...
    'SOLO_AI_WORLD_TICK_V1',
    'SOLO_PUBLISH_RECOVERY_V1'
));
...
CHECK (
    ...
    OR "outcome" IN (
        ...
        'ATTEMPT_MISSING',
        'NOTHING_TO_PUBLISH',
        'SOLO_OPENING_RECOVERED',
        'SOLO_ACTION_RECOVERED'
    )
);
...
CHECK (
    ...
    OR "dedupeKey" LIKE 'SOLO_AI_WORLD_TICK_V1:%'
    OR "dedupeKey" LIKE 'SOLO_PUBLISH_RECOVERY_V1:%'
);
