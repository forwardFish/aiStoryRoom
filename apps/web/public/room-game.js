...
function render(status = "") {
  ...
  if (requiresUnlock) {
    ...
    if (panel) panel.innerHTML = `<section class="room-unlock-gate" data-unlock-gate><h2>Free opening complete</h2><p>Rounds 1–${esc(access.freeRounds || 3)} are free. One participant unlocks the shared room for everyone; there is no per-round charge after unlock.</p><p><strong>${esc(access.requiredCredits || 100)} World Credits</strong> · Your available balance: <strong>${esc(access.balance ?? 0)}</strong></p><button class="btn primary" data-unlock-room>Unlock shared room</button></section>`;
  }
