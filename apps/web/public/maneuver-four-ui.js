...
export function renderFourManeuverPanel(view, state) {
  ...
  return `<section class="maneuver-panel maneuver-panel--mvp" data-testid="maneuver-panel" aria-busy="${busy}">
    ...
    <section class="maneuver-usage"><span>今日谋划</span><b>${number(panel.quota?.remaining)} / ${number(panel.quota?.perDay || 2)}</b><small>${busy ? "正在处理主动谋划，请稍候" : "未使用机会将在今日结束时失效"}</small></section>
    <div class="maneuver-action-list">${definitions.map(([type, label, count, description]) => {
      const section = panel[type] || {};
      const disabled = busy || Boolean(preview) || !section.enabled;
      const subtitle = busy ? "正在处理主动谋划……" : preview ? "请先确认或返回修改" : !section.enabled ? section.disabledReason || panel.disabledReason || description : description;
...
function renderWorkbench(view, state, type, busy) {
  ...
  if (type === "contact") {
    ...
    return `<section class="maneuver-workbench" data-testid="maneuver-contact-workbench"><div class="maneuver-workbench-head"><b>选择人物</b><small>列表里的人现在都可以交谈</small></div><div class="maneuver-option-list">${(panel.contact.options || []).map((item) => `<button type="button" class="maneuver-option-card ${draft.targetRoleKey === item.roleKey ? "selected" : ""}" data-contact-role="${escapeHtml(item.roleKey)}"${disabled}><span class="contact-avatar ${escapeHtml(item.portrait || "")}" aria-hidden="true"></span><span><b>${escapeHtml(item.displayName)}</b><small>${escapeHtml(item.publicIdentity)} · ${escapeHtml(item.relevance)}</small></span></button>`).join("")}</div><textarea id="contactMessageText" maxlength="200" placeholder="你想对他说什么？"${disabled}>${escapeHtml(draft.messageText || "")}</textarea><button id="maneuverSubmit" type="button"${disabled}>${target ? `生成发送预演` : "生成交谈预演"}</button></section>`;
  }
  if (type === "investigate") {
    const draft = drafts.investigate;
    return `<section class="maneuver-workbench" data-testid="maneuver-investigate-workbench"><div class="maneuver-workbench-head"><b>选择调查</b><small>调查内容由当前剧情决定</small></div><div class="maneuver-option-list">${(panel.investigate.options || []).map((item) => `<button type="button" class="maneuver-option-card ${draft.intentKey === item.intentKey ? "selected" : ""}" data-investigation-key="${escapeHtml(item.intentKey)}"${disabled}><span><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.summary)}</small></span></button>`).join("")}</div><button id="maneuverSubmit" type="button"${disabled}>生成调查预演</button></section>`;
  }
  if (type === "leverage") {
    const draft = drafts.leverage;
    const option = panel.leverage.options?.find((item) => item.leverageKey === draft.leverageKey);
    return `<section class="maneuver-workbench" data-testid="maneuver-leverage-workbench"><div class="maneuver-workbench-head"><b>选择筹码</b><small>筹码整局有限，确认后永久消失</small></div><div class="maneuver-option-list">${(panel.leverage.options || []).map((item) => `<button type="button" class="maneuver-option-card ${draft.leverageKey === item.leverageKey ? "selected" : ""}" data-leverage-key="${escapeHtml(item.leverageKey)}"${disabled}><span><b>${escapeHtml(item.label)}</b><small>${escapeHtml(item.description)}</small></span><em>${escapeHtml(item.consumptionLabel || "使用后消失")}</em></button>`).join("")}</div>${option?.requiresTarget ? `<div class="maneuver-target-list"><b>使用对象</b>${(option.targets || []).map((target) => `<button type="button" class="${draft.targetRoleKey === target.roleKey ? "selected" : ""}" data-leverage-target="${escapeHtml(target.roleKey)}"${disabled}>${escapeHtml(target.displayName)}</button>`).join("")}</div>` : ""}<button id="maneuverSubmit" type="button"${disabled}>${option ? `生成“${escapeHtml(option.label)}”预演` : "生成筹码预演"}</button></section>`;
  }
  const value = drafts.custom.customText || "";
  return `<section class="maneuver-workbench" data-testid="maneuver-custom-workbench"><div class="maneuver-workbench-head"><b>自拟谋划</b><small>写下一项当前身份和资源允许的行动</small></div><textarea id="customManeuverText" maxlength="200" placeholder="输入你的谋划……"${disabled}>${escapeHtml(value)}</textarea><span class="maneuver-counter">${value.length} / 200</span><button id="maneuverSubmit" type="button"${disabled}>生成谋划预演</button></section>`;
}
