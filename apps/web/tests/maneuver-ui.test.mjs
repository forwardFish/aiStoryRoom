...
test("主游戏右栏可以执行主动谋划并把结果写回同一消息流", async () => {
  ...
  await waitFor(() => storage.calls.filter((item) => item.kind === "maneuver").length === 1);
  ...
  assert.equal(root.querySelector('[data-testid="maneuver-contact-workbench"]'), null);
  root.querySelector('[data-maneuver-leverage="land_contract_fragment"]').click();
  await waitFor(() => storage.calls.filter((item) => item.kind === "maneuver").length === 2);
  assert.equal(storage.calls.filter((item) => item.kind === "maneuver").at(-1).input.maneuverType, "leverage");
  assert.equal(storage.calls.filter((item) => item.kind === "maneuver").at(-1).input.leverageKey, "land_contract_fragment");
...
test("派遣调查只在选择该类型后展示，并携带明确调查方向执行", async () => {
  ...
  assert.equal(storage.calls.at(-1).input.intentKey, "inspect_land_register");
});
