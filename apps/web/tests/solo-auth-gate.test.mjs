...
test("Play Solo redirects a signed-out player to login before role selection", async () => {
...
test("Play Solo verifies a cookie session before opening role selection", async () => {
  const requests = [];
  const harness = await authenticatedRouteHarness({
    signedIn: true,
...
test("every Solo entry uses the auth gate and duplicate-click guard", async () => {
  const source = await readFile(platformUrl, "utf8");
  const actions = source.slice(source.indexOf("const actions ="), source.indexOf("async function initializePlatform"));

  assert.match(actions, /solo:[\s\S]*runMutationOnce\("solo:caesar"[\s\S]*openAuthenticatedRoute\("\/role-select\?story=caesar"\)/);
  assert.match(actions, /"sangtian-solo":[\s\S]*runMutationOnce\("solo:sangtian"[\s\S]*openAuthenticatedRoute\("\/role-select\?story=sangtian"\)/);
  assert.match(actions, /"world-solo":[\s\S]*runMutationOnce\(`world-solo:\$\{worldId\}`[\s\S]*openAuthenticatedRoute\(`\/role-select\?story=\$\{encodeURIComponent\(worldId\)\}`\)/);
});
