...
test("auth page loads the public runtime configuration before its Google button", async () => {
  ...
  assert.match(html, /<script src="\/runtime-config\.js"><\/script>\s*<script type="module" src="\/world-catalog\.js[^"]*"><\/script>\s*<script type="module" src="\/room-lobby-live-client\.js[^"]*"><\/script>\s*<script type="module" src="\/platform\.js(?:\?v=[^"]+)?">/);
