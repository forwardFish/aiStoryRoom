...
const first = service.package("sangtian", "sangtian_v1_1");
const second = service.package("sangtian", "sangtian_v1_1");
assert.equal(first, second);
assert.equal(first.contract.worldId, "sangtian");
assert.deepEqual(first.contract.playableRoleKeys, ["zhejiang_governor", "xunfu", "county_magistrate"]);

const bound = service.forGame("sangtian", "sangtian_v1_1");
assert.equal(bound.stage(1).stageNumber, 1);
assert.equal(bound.isPlayableRoleKey("xunfu"), true);
assert.equal(bound.isPlayableRoleKey("merchant"), false);
