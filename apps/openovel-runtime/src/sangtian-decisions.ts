...
type SangtianDecisionContext = {
  workingSet: PartOneRuntimeWorkingSet & { kernelSelection?: KernelSelectionTrace };
...
export async function prepareSangtianDecision(
  ...
  if (
    ...
    const context = await currentSangtianDecisionContext(workspace, input.runId);
    if (context) assertSelectedOptionMatchesContext(input.selectedOption, context);
  }
...
export function nextSangtianOptions(
  ...
  const options = base.nextSangtianOptions(prepared);
  if (!options.length) return options;
  const context = decisionContextForCommittedEvent(
    prepared.package,
    prepared.settlement.proposedState,
...
export async function currentSangtianOptions(
  ...
  const options = await base.currentSangtianOptions(workspace, runId);
  if (!options?.length) return options;
  const context = await currentSangtianDecisionContext(workspace, runId);
  if (context) assertOptionsMatchContext(options, context);
  return options;
}
...
async function currentSangtianDecisionContext(
  ...
  const lastCommittedEventId = String(state.lastCommittedEventId || "").trim();
  ...
  const matches = events.filter((event) => event?.eventId === lastCommittedEventId);
  if (!matches.length) throw new Error("SANGTIAN_COMMITTED_EVENT_MISSING");
  if (matches.length > 1) throw new Error("SANGTIAN_COMMITTED_EVENT_DUPLICATE");
...
function decisionContextForCommittedEvent(
  ...
  if (Number(event.turnNumber || 0) !== Number(turnNumber || 0)
    || Number(state.turnNumber || 0) !== Number(turnNumber || 0)) {
    throw new Error("SANGTIAN_COMMITTED_EVENT_REVISION_MISMATCH");
  }
  if (state.lastCommittedEventId && state.lastCommittedEventId !== event.eventId) {
  ...
  if (pin.affordanceIds?.length) {
    const actual = workingSet.decisionAffordances.map((item) => item.affordanceTemplateId);
    if (!sameStringArray(actual, pin.affordanceIds)) {
      throw new Error("SANGTIAN_PINNED_AFFORDANCE_PAIR_MISMATCH");
    }
  }
  return { workingSet, pin };
}
...
function assertSelectedOptionMatchesContext(
  ...
  const expected = context.workingSet.decisionAffordances.find((affordance) => (
    affordance.affordanceTemplateId === selectedOption.id
  ));
  if (!expected) throw new Error("SANGTIAN_PINNED_SELECTED_OPTION_MISSING");
  if (selectedOption.effect?.decisionPointId !== context.pin.decisionPointId) {
    throw new Error("SANGTIAN_PINNED_SELECTED_OPTION_DECISION_POINT_MISMATCH");
  }
}
