# 单人主流程内嵌多人互动层：统一六席运行时模块化架构设计 v1.0
...
两种模式必须共用同一套内容、角色剧情、角色决策、行动合同、裁决、章节推进、
因果链和终局。`participantMode` 只决定大厅规则和席位由真人还是 AI 控制，不能决定
使用哪套剧情或裁决引擎。
...
→ 对方产生必须回应的义务
→ 对方真人回应，或 AI/超时策略代答
→ 原子更新双方个人状态、承诺、信任、关系和共同事实
→ AI 用短文本解释已提交结果
```

“必须回应”的定义：其他无关席位可以继续行动，但章节不能完成最终 Settlement，
直到所有 `RESPONSE_REQUIRED` 义务已经由目标席位回应，或由经过冻结、可追溯的超时
默认策略处理。页面提示不是权威，持久化的回应义务才是权威。
...
flowchart TD
    ...
    I --> Q{"是否影响另一席？"}
    Q -->|"否"| L["Working Ledger<br/>追加密封行动"]
    Q -->|"是"| OBL["创建 ResponseObligation<br/>并改变目标席待处理处境"]
    OBL --> TC{"目标席由谁控制？"}
    ...
    RS --> PROJ
    WF --> PROJ
    PROJ --> EX["AI 短文本解释已提交结果<br/>不能创造或改变事实"]
    EX --> END{"当前是否 N7？"}
    END -->|"否"| N2["从冻结权威打开下一章"]
    N2 --> N
...
| 共同开场白 | 相同 | 相同 | 是 |
| 当前角色剧情 | 按本人席位 | 按本人席位 | 是，`SeatTurnCore` |
| 当前角色决策 | 相同合同 | 相同合同 | 是，`SeatTurnCore` |
...
### M2：Seat Turn Core（保留现有主干）

- 用户目标：单人和多人拥有完全相同的“角色剧情 → 角色决策”。
...
### M3：Interaction Intent（保留并扩展窄合同）
...
### M6：Relationship & Commitment Authority（新权威/复用承诺）
...
### M7：World Fact Authority（从 WorldState 中收窄）
...
### M8：Unified Settlement Authority（扩展现有唯一裁决）
...
### M9：Audience Projection + Narrative（保留单向输出）
...
### M10：Chapter Progression & Finale（保留并改读新权威）
...
- 同一 content hash、seatId、个人状态、关系状态和世界事实下，Solo/多人产生相同
...
3. 共同开场、角色剧情、角色决策、行动合同、Settlement、N1～N7、Finale 共用；
