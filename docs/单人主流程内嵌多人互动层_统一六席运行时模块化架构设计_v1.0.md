...
- 每章可以包含多个剧情 Beat；中间 Beat 只推进故事和积累密封行动，章末才统一结算一次；
...
└─ 内阁财政：财政私人剧情 + 财政合法决策
             ↓
六席行动按 Beat 汇入同一本章 Working Ledger
             ↓
本章最后一个 Beat 完成后，只执行一次 Chapter Settlement
```
...
→ 当前 Beat 的必需行动与当前截止义务完成
→ 若本章还有下一 Beat：不执行 Settlement；一次 AI 生成下一段剧情和下一决策
→ 后续 Beat 继续把密封行动和回应追加到同一本章 Working Ledger
→ 只有本章最后一个 Beat 与全部章末义务完成
→ Chapter Settlement 对整章累积行动执行一次确定性、幂等、原子结算
→ 事务提交后，AI 一次生成章末文学剧情与章末总结
→ 玩家确认后才进入下一章
```

“必须回应”的定义：义务可以按内容策略要求在当前 Beat 截止，也可以被允许带入后续
Beat；它不阻塞无关席位的独立行动，也不应笼统锁死整个 Run。但章末 Settlement 开始
前，本章所有仍标记为 `RESPONSE_REQUIRED` 的义务必须已经由目标席位回应，或由经过
冻结、可追溯的超时默认策略处理。页面提示不是权威，持久化义务才是权威。

源行动必须先被 Working Ledger 接受，之后才能派生回应义务。禁止先创建义务、再尝试
写入源行动，否则源行动失败时可能留下没有权威来源的孤儿义务。

Narrator/DeepSeek 必须在数据库事务之外运行。中间 Beat 的一次调用读取本章 Working
Ledger 和当前 viewer-safe 材料，生成“下一段剧情＋下一决策”；章末的一次调用读取
已经提交的 Chapter Settlement，生成“章末文学剧情＋章末总结”。不增加单独的行动
后果改写调用，也不能创建事实、改变 Delta 或决定是否完成回应义务。
...
| N1-M3 | 通用 Beat 推进 | 中间 Beat 由本章 Working Ledger 和内容条件驱动；最后 Beat 才进入 Chapter Settlement |
...
目标状态必须拆为四个权威域，并由每章唯一一次 Chapter Settlement 原子提交：
...
flowchart TD
    ...
    P0 --> O["全员相同的共同长开场白"]
    O --> N["打开当前章节 N1～N7"]
    N --> B["打开本章当前 Beat<br/>一章包含多个剧情 Beat"]

    B --> V["按 viewerSeatId 投影<br/>当前角色剧情 + 当前角色决策 + 本人状态"]
    V --> C{"当前席位由谁控制？"}
    ...
    CONV{"当前 Beat 截止的必需行动与回应<br/>是否完成？"}
    CONV -->|"否"| V
    CONV -->|"是"| LAST{"是否本章最后一个 Beat？"}
    LAST -->|"否"| WP["Working Story Pack<br/>读取本章已密封行动、可见回应与下一 Beat 作者材料"]
    WP --> TURNGEN["事务外一次 AI 生成<br/>下一段剧情 + 下一决策；不结算"]
    TURNGEN --> B
    LAST -->|"是"| CHCONV{"本章全部必需行动与章末义务<br/>是否完成？"}
    CHCONV -->|"否"| V
    CHCONV -->|"是"| SET["唯一 Chapter Settlement<br/>对整章累积行动只结算一次"]
    ...
    PS --> PROJ["Audience-safe Chapter Result Projection"]
    RS --> PROJ
    WF --> PROJ
    PROJ --> EX["事务外一次 AI 生成<br/>章末文学剧情 + 章末总结"]
    EX --> CONFIRM["玩家查看总结并确认"]
    CONFIRM --> END{"当前是否 N7？"}
    END -->|"否"| N2["从冻结权威打开下一章"]
    N2 --> N
...
| 中间剧情 Beat | 追加本章 Working Ledger，不结算 | 各席追加同一本章 Working Ledger，不结算 | 是 |
| 章末裁决 | 本章末一次 Chapter Settlement | 本章末一次 Chapter Settlement | 是 |
...
flowchart LR
    ...
    Obligation --> BeatGate["Beat Progression Gate"]
    BeatGate -->|"中间 Beat"| WorkingProjection["Viewer-safe Working Story Pack"]
    BeatGate -->|"本章最后 Beat"| Settlement["Chapter Settlement Authority"]
    ...
    PlayerWrite --> Projection["Audience-safe Chapter Result Projection"]
    RelationWrite --> Projection
    FactWrite --> Projection
    WorkingProjection --> Narrative["Narrative Context / Narrator / Truth Guard"]
...
- 中间 Beat 只追加密封行动、回应和可见 Working Story Pack，不写最终个人/关系/世界 Delta；
- `Chapter SettlementAuthority` 是唯一能决定并提交整章最终变化的模块，每章只运行一次；
- 三类账本由同一事务/同一幂等 commit manifest 原子写入，但保持三个独立领域合同；
- 中间 Narrative 只能表达本章已密封行动和作者材料；章末 Narrative 只能解释已提交的
  Chapter Settlement，二者都不能补造权威事实；
...
- 输入及权威来源：仅章末 Chapter Settlement 的 `WorldFactDeltaV1`。
...
- 用户目标：一章内多轮剧情和决策累积完成后，只结算一次，并可靠地改变行动者、
  目标者、双方关系和共同事实。
- 唯一职责：在本章最后 Beat 后，根据整章密封行动、回应和冻结内容规则计算统一
  Delta，并以一次幂等事务原子提交。
- 明确不负责：写剧情文本、页面显示、玩家认证。
- 输入及权威来源：本章全部 Sealed Actions、已完成/默认的回应义务、章前权威快照、
  冻结规则和本章 Working Ledger hash。
...
- 聚焦测试：中间 Beat 不结算、每章只结算一次、整章确定性、原子性、幂等、冲突回滚、
  行动者/目标/关系/事实同时变化。
- 回滚：按 settlement contract 版本切回旧计算器；已提交记录不可重写。
- 玩家参与节点：本章最后一个决策完成后的章末真实结果与总结。
- 失败归属：最终变化不正确归本模块。

### USR-M9：Audience Projection + Narrative（保留单向输出）

- 用户目标：每个玩家只看到自己的状态和被授权事实，并得到简短清晰解释。
- 唯一职责：中间 Beat 从 viewer-safe Working Story Pack 一次生成下一段剧情和决策；
  章末从 viewer-safe Chapter Result 一次生成章末文学剧情和总结，并经 Truth Guard 校验。
- 明确不负责：裁决、状态写入、创建回应义务。
- 输入及权威来源：中间 Beat 使用冻结作者材料、本章已密封行动和当前 SeatId；章末使用
  三个权威只读快照、Chapter Settlement 证据和当前 SeatId。
...
- 聚焦测试：六席隔离、中间每段剧情一次调用且不触发 Settlement、章末一次调用只引用
  已提交事实、Provider 失败不损坏 Working Ledger 或 Chapter Settlement。
...
- 唯一职责：中间 Beat 只推进到下一 Beat；最后 Beat 的全部必需行动/回应完成后触发一次
  Chapter Settlement，展示章末总结并等待玩家确认，之后才打开下一章或进入 Finale。
...
1. 中间 Beat 只追加本章 Working Ledger，不输出或提交最终 Delta；
2. 本章最后 Beat 后，B0 规则根据整章密封行动输出个人、关系和世界事实三类 Delta；
3. 每章一次事务提交三类 Delta 和同一 commit manifest；
4. 保持旧 `WorldDelta` 兼容适配器只读，不成为第二权威。

验收门：中间任一 Beat 均未结算；章末一次结算综合本章行动改变相关席位，重放不重复
扣资源。
...
→ B 回应
→ 回应和后续 Beat 行动继续追加到本章 Working Ledger
→ 本章最后 Beat 完成后只执行一次 Chapter Settlement
→ Chapter Settlement 原子改变 A、B、A→B/B→A、承诺和共同事实
...
- 中间 Beat 只能推进故事并追加 Working Ledger，禁止提交最终个人/关系/世界 Delta；
- 所有章末必需席位和回应义务完成后，整章只能执行一次 Chapter Settlement；
- N1～N6 只能打开一个下一章；
- N7 只能生成一个 Finale；
...
理由是前者保持一个 Run、一条因果链、一个 Settlement Authority（每章执行一次）、
一套 N1～N7 和一个 Finale，同时让 Solo 与多人使用相同机制获得各席不同的角色体验；
后者会制造六条互相冲突的状态与因果权威，无法可靠实现“一个真人行动改变另一个真人
处境”。
