# AI 多人局｜数据库与存储架构设计文档 v2.3（持续叙事流修订版）
...
本版增加主游戏页持续叙事流所需的运行态投影：

```text
StoryEvent 仍是权威追加事件
NarrativeEntry 是公共前台投影，不新增表
StoryRun.stateJson 增加 activePrompt / pendingCriticalEvents
结果统一投影为 story + change_summary
关键事件延后通过 critical_event_deferred 追加事件持久化
公共选项只返回 optionKey + title
内部 description/gain/risk/statePatch 不得进入前台
```

关键事件回应属于固定 12 次主线决策中的一个槽位。

本版新增主动谋划的运行态、事件类型、API、事务和恢复规则。
...
前台统一显示“主动谋划 / 谋划机会 / 自拟谋划 / 执行谋划”，禁止使用 AP、行动力、行动点或筹谋。

## 0.0 工程基线优先级（v2.1 新增）

本文档受《AI 多人局｜MVP 唯一工程基线 v1.2》约束。当前正式锁定：

```text
前端继续使用 apps/web
运行态使用 StoryRun + StoryEvent
主动谋划作为 StoryEvent.type 表达，不新增独立 Maneuver 表
浏览器仅保存 runId，不保存权威游戏状态
不新增 StoryMessage / PlayerDecision 独立表
接真实模型后新增 AiTask
多人阶段才新增 StoryPlayer / MessageDelivery
```

PRD 中若仍出现 StoryMessage / PlayerDecision 或 apps/player-web / Next.js 迁移建议，均视为长期备选，不进入当前 MVP。
...
虽然当前可使用服务端 JSON 文件存储或数据库，但数据结构必须按未来后端抽象：
...
这样后续从服务端文件存储迁移到数据库时，只替换存储适配器，不重写页面和游戏引擎。
...
```text
阶段 0：服务端文件持久化或数据库，前端只保存 runId
阶段 1：正式数据库化，只建 StoryRun + StoryEvent
...
`StoryMessage` 和 `PlayerDecision` 不单独建表；分别使用 `StoryEvent.type=message` 与 `StoryEvent.type=decision` 表达。
...
```json
{
  ...
  "projection": {
    "uiProjectionVersion": "1.1",
    "latestNarrativeEventId": "evt_change_003"
  }
}
```
...
- `promptKind=critical_response` 在 MVP 中 `countAsMainDecision=true`；
- `pendingCriticalEvents` 只保存必要状态，正文来自 StoryEvent；
- 浏览器滚动位置不属于权威状态；
- 公共 NarrativeEntry 必须由事件流重新投影，不直接存为前台 HTML。
### 6.4 version 字段
...
```text
story_block                 连续故事正文
decision_prompt             主线或关键回应 prompt
decision                    玩家提交的最终决定
decision_result             规则与 AI 的结构化结果
change_summary              玩家可见变化
maneuver                    玩家主动谋划
maneuver_result             谋划结构化结果
leverage_used               筹码使用
pursuit_updated              推进目标更新
state_patch                 权威状态变化（hidden）
role_reaction               角色反应
critical_event              关键事件
critical_event_deferred     延后处理记录
hidden_thread               隐藏暗线
...
`NarrativeEntry` 不是数据库表。公共投影把以上事件组合为：
...
兼容旧事件时可保留 `messageType`，新代码优先使用 `type + payloadJson.view`。
...
不得再依赖 `messageType` 决定前端卡片样式；前端统一按 `NarrativeEntry.entryType` 渲染。
### 7.5 visibility 类型
...
```json
{
  ...
  "payloadJson": {
    ...
    "severity": "high",
    "requiresResponse": true,
    "responsePromptEventId": "evt_prompt_d5_1"
  }
}
```
...
{
  "day": 5,
  "type": "critical_event_deferred",
  "visibility": "private",
  "payloadJson": {
    "originEventId": "evt_critical_001",
    "status": "deferred"
  }
}
```

延后事件不完成 prompt、不增加 `decisionsCompletedToday`。
...
```json
{
  ...
  "payloadJson": {
    ...
    "customText": "派幕僚暗查驿站登记。",
    "opportunitiesBefore": 2,
    "opportunitiesAfter": 1
  }
}
```
...
- 所有权威变化与事件追加必须在一个事务中完成；
- 被 ActionGuard 拒绝的操作不得写入 decision/maneuver；
- 公共投影不得包含内部收益、风险、隐藏 patch 和私密推理；
- 主动谋划不改变 `decisionsCompletedToday`；
- critical_response 成功后必须改变 `decisionsCompletedToday`。
## 9. 第三张表：AiTask
...
3. 校验 eventId 是唯一 activePrompt
4. 校验 promptKind
5. 校验 customText 1—200 字
6. ActionGuard
7. 规则引擎与 AI 生成 resultStory / statePatch / visibleChanges
8. 更新 StoryRun.stateJson
9. 写入 decision / decision_result / state_patch / role_reaction
10. 必要时写入 critical_event
11. version + 1
12. 返回最新公共投影
```

若 `promptKind=critical_response`，成功后同样使 `decisionsCompletedToday +1`。
### 12.4 延后关键事件
...
```text
校验 eventId 是可延后的 critical_event
追加 critical_event_deferred
更新 pendingCriticalEvents 投影
version + 1
```

延后不完成 activePrompt，不消耗主线决策。
...
```text
1. 开启数据库事务
2. 读取 StoryRun，检查 status / currentDay / version
3. 检查 maneuverOpportunitiesRemaining > 0
4. 校验目标人物、筹码、意图和资源
5. 自拟谋划或语义模糊时调用 ActionGuard
6. 调用规则引擎 / AI 生成候选结果
7. normalize / schema validate / rule validate
8. 更新 worldState / roleState / relationships / clues / pursuits / leverage / fateSeeds
9. maneuversUsedToday + 1
10. remainingToday - 1
11. 写入 maneuver / maneuver_result 及必要衍生事件
12. version + 1
13. 返回最新公共视图
```
...
8. 将上一日未使用的谋划机会作废
9. 新一天将 maneuversUsedToday 重置为 0、remainingToday 重置为 2
10. 更新 StoryRun.stateJson
11. 返回最新 dashboard
```
...
```text
读取并锁定 StoryRun
校验 version / status / day / remaining
校验目标人物、筹码和意图
执行 Guard 与规则推演
更新 StoryRun.stateJson
追加 maneuver event
追加 maneuver_result message
追加可选 state_patch / role_reaction / fate_seed / leverage_used / pursuit_updated
maneuversUsedToday + 1
version + 1
```

以上必须在一个事务中完成。若任一步失败，不得扣减谋划机会。
...
当前前端不要直接操作 localStorage 权威状态。

```ts
interface StoryStorage {
  createRun(input: CreateRunInput): Promise<StoryView>;
  getRun(runId: string): Promise<StoryView>;
  getNarrativeEntries(runId: string, after?: string): Promise<NarrativeEntry[]>;
  submitDecision(input: SubmitDecisionInput): Promise<StoryView>;
  deferCriticalEvent(input: DeferCriticalEventInput): Promise<StoryView>;
  submitManeuver(input: SubmitManeuverInput): Promise<StoryView>;
  advanceDay(runId: string): Promise<StoryView>;
  finalize(runId: string): Promise<StoryView>;
}
```
...
公共前端使用 `NarrativeEntry[]`，不直接消费隐藏 StoryEvent。
## 17. 推荐目录结构
...
前端改为持续叙事时间线
固定顶部、左栏和右栏
实现 StoryEvent → NarrativeEntry 公共投影
stateJson 增加 activePrompt / pendingCriticalEvents
公共选项裁剪为 optionKey + title
实现结果故事 + change_summary
实现 critical_event / critical_event_deferred
实现局势记录投影与滚动恢复
保持 StoryRun / StoryEvent 两表模型
```
### P1：后端两表
...
实现 dashboard / decisions / maneuvers / advance-day / finalize
实现谋划机会扣减、跨日重置、筹码使用和谋划结果事件
...
```text
增加 AiTask
记录 ActionGuard / maneuverGuard / resolveDecision / resolveManeuver / roleReaction / finalJudgement 输入输出
实现失败重试
```
...
> **剧本用 JSON 文件，运行态用 StoryRun + StoryEvent；主线决策与主动谋划都作为事件流落账；AI 任务用 AiTask；多人权限用 StoryPlayer + MessageDelivery。整体按 openovel 的“事件日志 + 当前快照 + 后台账本 + 上下文卡片”思想设计，但用数据库实现 Web 多人局。**
