# AI 多人故事局 PRD 阶段化工程完整增强版 v5.0

> 本文档是在 `AI_multiplayer_story_RPG_MVP_Codex_PRD_v3_1_FINAL.md` 的基础上增强而成。  
> 处理原则：**不删减 v3.1 原工程 PRD，不弱化 MVP，可直接给 Codex/开发团队使用；在原文档前增加“最终版、MVP版、渐进版”的完整阶段化说明，并在原文档后补充长期 GitHub 式故事世界共创路线。**

---

# A. 总体结论：最终版、MVP版、渐进版的关系

## A.1 为什么必须分三个层次

这个项目不能只写成一个简单 MVP，也不能一开始按最终平台开发。

如果一开始只做“AI 多人剧情局”，产品容易停留在一次性娱乐：用户玩完一章，觉得新鲜，但没有持续沉淀。如果一开始直接做“AI 群像故事世界共创平台”，开发量、用户理解成本、内容治理、AI 成本都会失控。

因此本项目必须拆成三个层次：

1. **最终版**：明确长期方向，指导技术架构不要走偏。
2. **MVP版**：明确当前必须开发的最小闭环，保证能上线、能验证、能控制成本。
3. **渐进版**：明确从 MVP 如何一步步过渡到最终形态，避免“做完 MVP 后不知道下一步”。

三者关系如下：

```txt
最终版：前台像游戏，后台像 GitHub 的故事世界共创平台
        ↑
渐进版：从故事局 → 故事世界主页 → 角色线 → 正史/支线 → 创作者生态
        ↑
MVP版：固定模板 + 角色行动 + AI 导演结算 + 三个回响 + 多 POV 章节
```

## A.2 产品最终一句话

> **一个故事世界，很多人的主角线。AI 把大家的选择、角色线、支线贡献合并成正史。**

这句话比“AI 写小说”“AI 剧本杀”“AI 多人故事局”更接近长期方向。

但 MVP 对外不建议直接讲得这么复杂。MVP 对外仍然使用：

> **和朋友一起玩出一章小说。**

用户先通过“玩一章”进入，后续再逐步感知：我的角色线还在、我的支线可以继续、我的贡献可以进入正史。

## A.3 前台像游戏，后台像 GitHub
...
前台必须低门槛、强反馈、有悬念、有行动、有结果。用户不能一上来面对复杂的“仓库、分支、合并、正史”。
...
```txt
StoryWorld 故事世界 = Repo 仓库
CharacterArc 角色线 = Branch 分支
PlayerAction 玩家行动 = Commit 提交
Contribution 支线贡献 = Pull Request 候选并入
CanonChapter 正史章节 = Merge 后的主线内容
StoryBranch 支线 / 平行线 = Branch / Fork
WorldIssue 未解谜题 = Issue
WorldOwner 世界主 = Maintainer
AI 合并编辑 = CI + Reviewer + Formatter
```

但这些概念在 MVP 只做底层预留，不暴露复杂操作。
...
最终形态中，一个用户可以创建一个故事世界，其他用户可以进入这个世界，选择角色线、补充支线、推动事件、提交番外、开启平行分支。AI 负责校验世界观、检查冲突、整理时间线、生成正史章节，并帮助世界主完成内容治理。

## B.2 最终版核心用户

### 1. 普通玩家

目标：进入故事，选择角色，做行动，获得剧情反馈和个人章节。
...
目标：不一定参与创作，只阅读热门世界、追角色线、收藏章节、参与投票。
...
### B.3.2 角色命运线 CharacterArc

每个角色都不只是身份卡，而是一条可以长期发展的命运线。
...
```txt
玩家回来不是为了“再玩一局”，而是为了继续自己的角色线。
```

### B.3.3 AI 导演 DirectorAI

最终版 AI 不只是写小说，而是承担五个角色：

1. **行动裁判**：判断玩家行动是否合理。
2. **世界状态管理者**：维护事实、线索、关系、危险等级。
3. **群像导演**：保证每个玩家都有主角感。
4. **合并编辑**：将多人行动、支线、贡献整理成统一文风。
5. **正史校验器**：判断内容是否与世界观、时间线、角色设定冲突。

### B.3.4 正史与分支 Canon & Branch
...
1. **普通玩家订阅**：更长章节、更高质量模型、更大角色记忆。
2. **故事局付费**：高级故事局、私密世界、多角色位。
3. **世界主 Pro**：创作者工具、模板管理、数据看板、AI 合并额度。
4. **内容付费**：付费支线、付费番外、付费角色线。
5. **平台分成**：世界主收益分成。
6. **IP 孵化**：热门世界改编漫画、短剧、动画。

## B.5 最终版不能现在就做的原因

最终版虽然方向清楚，但不适合当前一次性开发：
...
# C. MVP版：当前必须实现的最小可验证版本

## C.1 MVP 的核心目标

MVP 不是做完整平台，而是验证一个关键问题：

> **用户是否愿意进入一个故事世界，选择角色，提交行动，看 AI 导演结算，并因为“我的行动影响了别人和世界，最后生成我的 POV 章节”而愿意分享、复玩、继续下一章。**
...
```txt
夜班店员：你在收银机里发现一枚不属于今晚账目的旧硬币。
外卖骑手：你接到一份没有平台记录的订单，收货人是你自己。
实习记者：你找到父亲十年前留下的旧新闻，照片里出现午夜便利店。
```
...
```txt
因为你触碰了硬币，外卖骑手的订单备注发生变化。
因为记者公开旧新闻，便利店老板开始干预监控。
```

### 6. 多 POV 章节 + 个人故事卡

章节结果不是单一主角小说，而是多 POV 群像章节。
...
1. 微信登录 mock。
2. 首页：开一局故事、单人试玩、加入朋友的故事。
3. 世界模板：午夜便利店、青云宗门、穿越荒村。
4. 创建 StoryRun。
5. 生成第一章 ChapterSandbox。
6. 生成 StoryRole + CharacterArc。
7. 角色选择页：选择命运线。
8. 角色卡页：展示公开身份、个人目标、命运问题、隐藏秘密。
9. 故事局房间页：当前剧情、目标、我的命运问题、线索、行动入口、玩家状态。
10. 行动提交页：行动类型、对象、方式、目的、风险、自定义补充。
11. ActionGuard：拦截越权、宣布结果、操控他人、跳过剧情。
12. AI 导演结算：输出行动结果、三个回响、跨角色影响、线索、关系、危险等级。
13. 世界状态更新：facts、clues、relations、dangerLevel、currentNode。
14. 下一节点生成。
15. 每章 3-5 个节点。
16. 多 POV 章节生成。
17. 个人故事卡生成。
...
目标：让用户 30 秒内明白这是“可以玩的故事”。

模块：

```txt
顶部主文案：和朋友一起玩出一章小说
副标题：选择角色，做出行动，AI 导演把你们的选择写成故事。
按钮：单人试玩 / 开一局故事 / 加入朋友的故事
模板卡：午夜便利店、青云宗门、穿越荒村
```
...
GET /api/nodes/:nodeId/my-echoes
GET /api/story-runs/:runId/fate-net-lite
```
...
export type ResolveNodeOutput = {
  ...
  clueChanges: Array<any>;
  relationChanges: Array<any>;
  dangerDelta: number;
...
export type GenerateChapterOutput = {
  ...
  highlights: Array<{ roleId: string; text: string }>;
  keyChoices: string[];
  nextHook: string;
};
```
...
```txt
你是 AI 多人故事局的行动裁判。
你的任务是判断玩家提交的行动是否允许进入故事结算。
玩家只能提交“自己的行动意图”，不能宣布结果，不能操控他人，不能跳过当前剧情阶段，不能新增破坏世界观的大设定。
输出必须是合法 JSON。
```
...
```txt
你是 AI 多人故事局的群像导演和裁判。
你要根据当前节点、玩家行动、世界规则、角色秘密、已知线索，结算本节点。
每次结算必须做到：
1. 保留每个玩家行动的核心意图。
2. 判断行动结果：成功、失败、部分成功、成功但付出代价、触发风险。
3. 更新线索、关系、危险等级和世界状态。
4. 输出每个活跃角色的三个回响：个人回响、他人回响、世界回响。
5. 至少产生 1 个跨角色影响。
6. 不替玩家做重大决定。
7. 不剧透未公开秘密。
8. 不跳到大结局。
...
```txt
你是章节编辑。
你要把本章 3-5 个剧情节点的公共叙事、玩家行动、行动结果、线索变化、关系变化、三个回响、跨角色影响，整理成一章多 POV 群像小说。
要求：
1. 第三人称。
2. 统一文风。
3. 每个玩家至少有一个 POV 片段或高光段落。
4. 保留本章关键选择和跨角色影响。
5. 不新增破坏后续剧情的大设定。
6. 不剧透未公开秘密。
7. 结尾必须有下一章悬念。
...
## C.11 MVP Sprint 开发计划

### Sprint 1：工程骨架与模板数据

目标：本地能跑起 API、小程序、后台，并有 3 个世界模板。
...
配置 Next.js admin
实现 shared types
实现 WorldTemplate seed
实现 mock 微信登录
实现模板列表 API
实现首页基础 UI
```

### Sprint 2：StoryRun 与角色命运线
...
未行动玩家不会永久阻塞故事
AI 失败有 fallback
后台可查看故事局、行动、结算、AI 日志、审核日志
核心埋点能形成漏斗
```

---

# G. 本文档后续保留 v3.1 原工程 PRD 全文

下面完整保留原 `AI_multiplayer_story_RPG_MVP_Codex_PRD_v3_1_FINAL.md` 内容，作为 Codex 开发执行细节。上面的 A-F 是阶段化增强说明，不替代 v3.1，而是作为 v3.1 的上层产品路线与工程约束。
