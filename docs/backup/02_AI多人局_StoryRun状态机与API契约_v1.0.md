# AI 多人局｜StoryRun 状态机与 API 契约 v1.0
...
| 当前状态 | GET run/messages/dashboard | 提交决策 | advance-day | finalize | 创建新局 |
|---|---:|---:|---:|---:|---:|
| created | 允许 | 禁止 | 禁止 | 禁止 | 允许 |
| awaiting_decision | 允许 | 允许（唯一活动事件） | 禁止 | 禁止 | 允许 |
| resolving | 允许 | 禁止/幂等查询 | 禁止 | 禁止 | 允许 |
| awaiting_day_advance | 允许 | 禁止 | 允许 | 禁止 | 允许 |
| awaiting_finalization | 允许 | 禁止 | 禁止 | 允许 | 允许 |
| finished | 允许 | 禁止 | 禁止 | 禁止 | 允许 |
| abandoned | 允许 | 禁止 | 禁止 | 禁止 | 允许 |
| error_recoverable | 允许 | 禁止 | 禁止 | 禁止 | 允许 |
...
所有修改 StoryRun 的请求必须携带当前 `version`：
...
| 状态 | 页面表现 |
|---|---|
| awaiting_decision | 恢复当前消息和选项 |
| resolving | 显示推演中并轮询 GET run |
| awaiting_day_advance | 显示日终回响和“进入下一天” |
...
- 第 1—5 天推进后进入下一天 `awaiting_decision`；
- 第 6 天推进后进入第 7 天 `awaiting_finalization`。
