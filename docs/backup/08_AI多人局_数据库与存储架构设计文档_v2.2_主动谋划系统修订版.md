# AI 多人局｜数据库与存储架构设计文档 v2.2（主动谋划系统修订版）
...
本文档受《AI 多人局｜MVP 唯一工程基线 v1.1》约束。当前正式锁定：
...
```json
{
  ...
  "pursuits": [
    {
      "key": "investigate_xunfu_merchant_pact",
      "title": "查清巡抚与商会旧约",
      "current": 1,
      "target": 3,
      "status": "active"
...
decision_result     主线决策结果消息
maneuver_result     主动谋划结果消息
day_end             日终回响
...
```json
{
  ...
  "payloadJson": {
    "title": "幕僚回报：驿站登记被改写",
    "narrative": "驿站登记显示，巡抚奏疏由其师爷亲自送出，但其中一页有重新誊写的痕迹。",
    "changes": {
      "newClues": ["被改写的驿站登记"],
      "pursuitUpdates": [
        {
...
- 成功谋划必须同时更新 StoryRun 快照和追加 StoryEvent；
- 被 ActionGuard 拒绝的谋划不得写入 `maneuver` 事件；
- 每次成功谋划只扣减一次机会；
- 主动谋划不改变 `decisionsCompletedToday`；
- 未使用的谋划机会在 `advance-day` 时直接作废并重置。
...
interface StoryStorage {
  ...
  submitManeuver(input: SubmitManeuverInput): Promise<StoryDashboard>;
...
浏览器只保存当前 `runId`，所有权威状态通过 `/api/v4/story-runs/...` 获取。`LocalStoryStorage` 仅允许用于独立单元测试，不作为正式运行方案。
...
在 stateJson 中加入 maneuverState / pursuits / availableLeverage
保留当前页面功能
```
