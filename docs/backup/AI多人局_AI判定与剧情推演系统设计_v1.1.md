...
> v1.1 更新重点：优先解决三个生死风险：用户感知不到因果、因果账本缺失导致 AI 临时编、角色反应像 NPC 工具人。
...
本版不是重写原系统，而是在 v1.0 的基础上，把三个核心风险转成可执行的系统约束。

### 风险一：用户感知不到因果

解决方式：增加 `CausalVisibilityEngine`，并强制每次决策后输出“可见因果卡”。
...
玩家不能只看到数值变化。数值变化必须被翻译成因果解释。

---

### 风险二：因果账本没做，AI 临时编

解决方式：把 FateSeed、EvidenceLedger、ResponsibilityLedger、NarrativeFrame、DaySummary 作为 MVP 必须保存的运行态数据。
...
### 风险三：角色没有真实动机，像 NPC 工具人

解决方式：增加 `RoleDecisionModel`，并要求 `RoleReactionEngine` 在生成角色行动前，先生成角色内部判断。
...
角色不是剧情工具，而是有身份、利益、恐惧、误判和底牌的局中人。
...
因此，每次关键决策后，前台必须展示“可见因果”，而不是只展示剧情段落或数值变化。
...
如果前台没有让玩家看见因果，后台因果账本再完整也无法形成游戏体验。
...
所以 `RoleReactionEngine` 的输出中必须包含 `privateReasoningSummary`。这段内容默认不展示给玩家，但必须写入后台事件，用于后续因果解释和调试。

---

### 1.7 AI 输出只能建议状态变化，系统负责最终落账

AI 输出中的状态补丁、证据更新、责任更新、叙事定性都只是候选结果。
...
如果 AI 输出不合法，系统应使用 fallback 规则：

```text
1. 保留玩家选择事件
2. 使用预设状态补丁
3. 生成保守结果消息
4. 标记 AiTask failed / fallback
5. 不生成无法验证的因果伏笔
```
...
↓
CausalVisibilityEngine：生成玩家可见因果卡
↓
FateSeedEngine：生成 / 触发命运伏笔
↓
...
↓
RoleDecisionModel：读取角色目标 / 恐惧 / 误判 / 决策偏好
↓
RoleReactionEngine：生成其他角色反应
↓
CausalRecallEngine：在伏笔触发时生成因果回溯消息
↓
...
```text
1. CausalVisibilityEngine 负责让玩家看见因果。
2. RoleDecisionModel 负责让角色不像 NPC。
3. CausalRecallEngine 负责在未来触发时告诉玩家：这件事来自你之前的选择。
```
...
## 3.3A CausalVisibilityEngine：可见因果展示引擎

### 作用

`CausalVisibilityEngine` 负责把后台因果账本翻译成玩家能看懂的前台反馈。

它解决的问题是：

```text
玩家不是看不懂剧情，而是看不出自己的选择到底改变了什么。
```

因此，每次关键决策后，系统必须生成一张“可见因果卡”。
...
```json
{
  "visibleCausalCard": {
    "decisionTitle": "追加密奏",
    "decisionSummary": "你没有截留巡抚奏疏，而是另写密奏给皇帝，说明浙江民心和粮价已有裂痕。",
    "personalEcho": "你为自己留下了未来解释权，但也留下了越级自保的痕迹。",
    "othersEcho": [
      {
        "roleKey": "xunfu",
        "text": "巡抚会意识到你没有拦他，却在京师留了后手。"
      }
    ],
    "worldEcho": "京师将收到两份口径不同的浙江奏报，司礼监开始注意浙江银路。",
    "stateChangesText": [
      "皇帝信任 +4",
      "内阁疑心 +2",
      "巡抚敌意 +3",
...
【个人回响】
你保留了未来解释权：如果粮价和民怨后来坐实，你可以证明自己早已预警。

【他人回响】
巡抚会意识到，你没有公开拦他，却在京师留了另一套说法。

【世界回响】
京师将收到两份口径不同的浙江奏报。司礼监会开始注意浙江内部并不一致。
...
### 3.7A RoleDecisionModel：角色决策模型

`RoleDecisionModel` 不是普通角色设定，而是角色每次行动前的判断依据。
...
《桑田诏》MVP 至少要配置以下角色的 `RoleDecisionModel`：
...
### v1.1 强制要求：角色反应必须先生成内部判断

`RoleReactionEngine` 不允许直接输出行动。
...
```json
{
  ...
  "privateReasoningSummary": "巡抚判断，如果自己不先把改桑有成效送到京师，等暗账浮出后就只剩解释。他不能停，必须先定义局势。",
  "chosenAction": "越级急奏内阁，只写改桑进展，不写粮价和田契争议。",
  "surfaceReason": "朝廷催银甚急，浙江不可迟疑。",
  "hiddenIntent": "把自己绑定为新政功臣，同时暗示总督府持重拖慢。",
  "messageToPlayer": {
    "title": "巡抚急奏北上",
    "narrative": "驿站快马离开杭州府。有人看见巡抚幕僚亲自护送急奏北上，奏中只写改桑已有成效，不提粮价和田契争议。"
  },
  "statePatch": {
    "巡抚敌意": 8,
    "内阁疑心": 5
  ...
  "newFateSeeds": [],
  "sourceEventIds": ["evt_day2_county_letter", "evt_day3_secret_memorial"]
}
```
...
## 3.10A CausalRecallEngine：因果回溯引擎

### 作用

当 FateSeed 被触发时，`CausalRecallEngine` 负责生成玩家可见的“因果回响”消息。

它解决的问题是：

```text
后果不是突然发生的，而是来自玩家之前的选择。
```
...
```text
1. FateSeed 从 dormant 变为 activated_help
2. FateSeed 从 dormant 变为 activated_backfire
3. 某个 EvidenceItem 被其他角色使用
4. 某个 ResponsibilityNode 的 dominantFrame 发生变化
5. 某个 NarrativeFrame 被京师 / 内阁 / 司礼监接受
```
...
```json
{
  "causalRecallMessage": {
    "title": "因果回响：密奏被重新定性",
    "originEventIds": ["evt_day3_secret_memorial"],
    "recallText": "这件事并非凭空而来。第 3 天，你没有截留巡抚急奏，而是追加密奏。当时这一步帮你保留了解释权。",
    "reframedBy": "浙江巡抚",
    "newFrame": "总督明面不拦，暗中另立口径，是借谨慎之名拖慢国策。",
    "currentPressure": "内阁开始怀疑你越级自保。",
...
```text
第 3 天你的密奏削弱了巡抚急奏，但也给了巡抚一个说法：总督绕过内阁、另立口径。第 5 天，他把这个说法递给内阁，试图反咬你拖延国策。
```
...
5. CausalVisibilityEngine 生成玩家可见因果卡
6. FateSeedEngine 生成 / 更新 / 触发命运伏笔
7. EvidenceLedger 更新证据流向
8. ResponsibilityLedger 更新责任流向
9. NarrativeFrameEngine 生成多方定性
10. RoleDecisionModel 读取角色目标、恐惧、误判和行动偏好
11. RoleReactionEngine 生成其他角色反应
12. CausalRecallEngine 在触发旧伏笔时生成因果回溯消息
13. StoryEventWriter 写入事件流
14. Dashboard 更新右侧状态、因果回响、痕迹和风险
15. 前台只展示玩家可见内容
```
...
每次决策后 AI 必须输出严格 JSON。v1.1 增加 `visibleCausalCard`、`causalRecallMessages`、`roleDecisionTrace` 三类字段。
...
```text
1. visibleCausalCard 必须存在，不能为空。
2. 每个 statePatch 必须能被解释到 visibleCausalCard 的某一项。
3. 每个 fateSeed 必须有 originEventId。
4. 每个 evidenceLedgerUpdate 必须说明 holderRoles / knownByRoles。
5. 每个 responsibilityLedgerUpdate 必须说明 issue 和 possibleResponsibleRoles。
6. 每个 narrativeFrame 必须说明不同角色如何定性同一事件。
7. 每个 roleReaction 必须包含 privateReasoningSummary。
8. 每个 causalRecallMessage 必须引用 originEventIds。
9. 前台不能展示 hiddenMeaning、完整 trigger condition、完整责任权重。
10. AI 输出不能直接作为数据库事实，必须经过 normalize / validate / apply。
```
...
```text
个人回响：玩家自己的资源、身份、清算风险、解释权、命运债变化。
他人回响：其他角色如何重新判断玩家。
世界回响：国库、民心、粮价、奏报、银路、军心等公共局势变化。
留下的痕迹：证据、口径、记录、目击、承诺、传话、账册、密奏。
潜在风险：模糊提示，不暴露未来具体触发条件。
```
...
## 11.6 CausalVisibility Prompt

```text
你是 AI 多人局的可见因果展示引擎。

你的任务不是继续写小说，而是把本次玩家决策造成的因果变化翻译成玩家能看懂的前台反馈。

你必须输出：
1. decisionTitle：玩家这次选择的标题
2. decisionSummary：玩家到底做了什么
3. personalEcho：这一步如何影响玩家自己
4. othersEcho：这一步如何影响其他角色的判断
5. worldEcho：这一步如何影响世界局势
6. stateChangesText：状态变化，用自然语言解释
7. tracesLeft：这一步留下了什么痕迹
8. potentialRisks：未来可能被谁重新定性的模糊风险
9. playerFacingHint：一句话告诉玩家这一步的意义

注意：
- 不能暴露完整 FateSeed 触发条件。
- 不能暴露其他角色完整隐藏动机。
- 不能只写数值变化。
- 每个状态变化都必须有因果解释。
```
...
## 11.7 RoleDecisionModel Prompt

```text
你是 AI 多人局的角色决策模型构建器。

请为目标角色生成一套可用于推演的 RoleDecisionModel。
...
注意：
角色不是 NPC 工具人。
角色必须有自己的身份、利益、恐惧、误判和底牌。
角色不能知道不该知道的信息。
角色的行动应当对自己有利，但未必对全局有利。
```

---

## 11.8 CausalRecall Prompt

```text
你是 AI 多人局的因果回溯引擎。

当某个 FateSeed、证据、责任流或叙事定性在后续剧情中触发时，你要生成玩家可见的因果回响。
...
即使第一版不拆表，也必须在 `StoryRun.stateJson` 中保存：
...
```text
EvidenceLedger 深化
ResponsibilityLedger 深化
NarrativeFrameEngine 深化
CausalRecallEngine 多样化
角色误判模型增强
多角色消息转译
```
...
这个 AI 判定系统是否成立，看 8 件事：

```text
1. 每次关键决策后，前台必须展示“你的决定 / 个人回响 / 他人回响 / 世界回响 / 状态变化 / 留下的痕迹 / 潜在风险”。
2. 玩家第 1 天的选择，第 4 天能被合理触发。
3. 同一个选择既可能帮助，也可能反噬。
4. 每次反噬都能说清楚 originEventId。
5. 角色反应必须包含 privateReasoningSummary，不能像系统随机安排。
6. 角色只能基于自己知道的信息、合理推测和误判行动。
7. 最终结局能解释关键三手、吞噬三手、命运债。
8. 玩家玩到第 4 天时，能说出：我前面某一步，现在开始帮我，也开始害我。
```
...
1. 玩家只觉得自己读了一堆 AI 剧情。
2. 玩家看不出数值变化和自己选择的关系。
3. 后续反噬没有引用前面的 originEventId。
4. 角色突然知道自己不该知道的信息。
5. 巡抚、县令、商会、司礼监像工具人，没有自己的利益逻辑。
6. 最终结局像 AI 总结作文，不能解释哪几步救了玩家、哪几步害了玩家。
```
...
说明体验还没有从“AI 互动小说”升级成“AI 动态局势推演游戏”。

---

## 15. 一句话总结

> **AI 剧情推演的核心不是让模型自由写故事，而是让系统每天记录玩家留下的痕迹，让角色基于自己的身份和利益反应，再把这些痕迹在正确时间变成玩家能看见的机会、罪名、证据、恩情或灾祸。**
