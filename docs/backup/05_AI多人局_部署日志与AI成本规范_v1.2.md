...
本版将持续叙事流、关键事件回应和公共投影纳入部署与可观测性规范。

新增约束：

```text
主线决策、关键事件回应、自拟谋划输入最大 200 字
关键事件回应占用固定 12 次主线决策中的一个槽位
不会额外增加主线推演预算
前台 resolving 只显示“AI 正在推演局势……”
AI fallback 成功时对普通用户静默
公共投影不得包含内部收益、风险、状态补丁预测和私密推理
```
...
entryType = story | change_summary | critical_event
```
## 1. 环境分层
...
```text
12 次主线 prompt resolve：最多 12 次主要调用
  └─ promptKind 可以是 main_decision 或 critical_response
12 次主动谋划 resolve：最多 12 次主要调用
6 次日终摘要：最多 6 次
1 次最终裁决：最多 1 次
自定义主线 prompt ActionGuard：最多 12 次轻量调用
自拟谋划 ActionGuard：最多 12 次轻量调用
------------------------------------------------
理论硬上限：55 次 provider 调用
```

关键事件回应不额外增加主线次数：它必须占用当日尚未完成的主线决策槽位。

推荐优先规则：

- 预设 A/B/C 不单独调用 ActionGuard；
- 点击建议后即使用户补充文本，只在语义明显扩展时调用 Guard；
- 结构化人物交谈、调查和筹码使用优先走确定性 Guard；
- 只有自定义文本或语义模糊时才调用轻量 AI Guard；
- 每个 prompt 最多 1 次主要生成调用；
- 每个主动谋划最多 1 次主要生成调用；
- 一次输出同时返回 `resultStory + visibleChanges + hiddenEffects`；
- 谋划机会未使用时不得为了补内容主动调用模型。
### 3.2 Token 硬限制
...
- 保留玩家原始主线决策、关键事件回应或主动谋划；
- 使用预设或规则计算的状态补丁；
- 主动谋划 fallback 仍正确扣减 1 次谋划机会；
- 生成保守但完整的 `resultStory`；
- 存在玩家可见变化时生成 `visibleChanges`；
- 后台生成可追溯因果记录与必要 FateSeed；
- 生成符合角色默认惯性的最小反应；
- 允许继续下一决策、下一次谋划和下一天；
- 最终裁决仍可完成。
...
普通用户不显示“AI 失败”“规则模板完成”等技术文案。前台仍按正常顺序展示：
...
[ ] 所有 P0 测试通过
[ ] 主线决策、回应和谋划输入均限制为 200 字
[ ] resolving 公共投影只显示“AI 正在推演局势……”
[ ] AI 双失败 fallback 全流程通过且普通用户不看到技术失败文案
[ ] resultStory 与 visibleChanges Schema 校验生效
[ ] 公共选项投影只包含 optionKey + title
[ ] 公共响应不包含 internalGain / internalRisk / statePatch 预测
[ ] 关键事件 defer 事件可持久化并恢复
[ ] 备份任务已运行
[ ] 告警渠道已验证
[ ] 公共响应私密字段扫描通过
[ ] 单局调用和成本硬上限生效
