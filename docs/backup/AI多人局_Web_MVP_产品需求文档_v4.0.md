...
```text
新建分支：feature/web-mvp-message-driven
将 apps/web 改造成正式 Web MVP
或新增 apps/player-web 作为正式 Web 前端
```
...
POST /api/v4/story-runs/:runId/messages/:messageId/decisions
...
```text
把 v4 PRD 放入 docs/
新建 feature/web-mvp-message-driven 分支
暂停 miniprogram 开发
确认 apps/web 或 apps/player-web 方案
```
...
```text
新增 StoryMessage / PlayerDecision
新增 v4 API
实现 demo run
...
优先完成：
1. 新增或重构 Web 前端，建议 apps/player-web 或重构 apps/web。
2. 实现三栏游戏主页面：
   - 左侧：我的身份、目标、资源、筹码、风险
   - 中间：剧情消息流 + 当前决策区
   - 右侧：世界状态、人物关系、最新变化、当前风险
3. 新增 StoryMessage / PlayerDecision。
4. 新增 v4 API：
   - GET /story-runs/:runId/messages
   - POST /story-runs/:runId/messages/:messageId/decisions
   - GET /story-runs/:runId/dashboard
   - POST /story-runs/:runId/advance-day
   - POST /story-runs/:runId/finalize
