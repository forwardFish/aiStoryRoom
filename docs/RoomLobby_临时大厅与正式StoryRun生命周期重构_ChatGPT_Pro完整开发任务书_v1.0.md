...
### 1.1 项目所有者唯一硬验收门：两个真实用户进入同一房间

本任务最终是否通过，只看下面这条真实用户闭环是否成立。任何单元测试、接口测试、HTTP 200、数据库行、自述或单账号页面演示都不能替代：

1. 使用真实测试用户 A，通过现有正式页面创建一个临时大厅，并成功进入该大厅的现有房间/选角页面。
2. 使用与 A 完全隔离的真实测试用户 B，在 `Open Rooms` 看到 A 创建的大厅，点击 `Join` 后成功进入同一个大厅。
3. A 的大厅出现在 A 的 `My Rooms`，操作为 `Open`；不能让 A 对自己的大厅执行 `Join`。
4. A、B 的服务端投影必须具有同一个 Lobby ID，并且双方页面都能看到两名真人的最新成员/席位状态。
5. A、B 分别选择不同的六席角色后，页面总席位仍然只有六个；真人替换对应 AI，绝不能出现 `7 / 6 players`。
6. A、B 分别刷新页面后，仍然回到同一个大厅，成员关系和已占角色由 Supabase 权威恢复，不能依赖单一浏览器内存或 localStorage 伪造。

验收必须使用两个隔离的浏览器会话或两个不同浏览器配置文件，不能在同一认证会话中切换显示名称冒充两个用户。测试数据必须通过正式 UI/API 创建，不得直接插入数据库来绕过 Join 流程。

必须保存以下证据：

- A 创建并进入大厅的页面截图、关键网络请求和 Lobby ID。
- B 在 Open Rooms 看到大厅、点击 Join 并进入的页面截图、关键网络请求和相同 Lobby ID。
- A/B 同时存在于同一 Lobby 的玩家安全 API 投影；数据库证据只作辅助，不可替代页面流程。
- 两端刷新后的恢复证据，以及页面不再出现 `7 / 6` 的截图。

只要 B 无法看到、Join 报错、进入了不同 Lobby、双方不能互相看到、刷新后丢失，或仍出现七名参与者，本任务整体结论就是 `FAIL`，不得以其他测试通过抵消。
...
- 必须首先通过第 1.1 节的“两个真实用户进入同一房间”一票否决验收门。
...
项目所有者唯一硬验收门：
- 使用两个完全隔离的真实测试用户 A/B 和现有正式页面。
- A 创建并进入大厅；A 的大厅只在 A 的 My Rooms 中显示 Open。
- B 在 Open Rooms 看到 A 的大厅，点击 Join 后进入与 A 相同的 Lobby ID。
- A/B 双方都能看到两名真人的最新成员/席位状态。
- A/B 分别占用六席中的两个角色，真人替换 AI，总席位仍为六，绝不能出现 7 / 6。
- 双方刷新后仍在同一大厅，状态从 Supabase 恢复。
- 必须保存两端页面截图、关键网络请求和相同 Lobby ID；不能用 HTTP 200、数据库直插、单账号或 mock 代替。
- 任一步失败，整体结论为 FAIL，不得声称完成。
